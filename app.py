...
class SimpleProtobuf:
    ...
    def create_login_payload(open_id, access_token, platform):
        if not game_version:
            raise Exception("game_version.py missing")
...
def send_once(remote_ip, remote_port, payload_bytes, recv_timeout=8.0):
...
def index():
    try:
        ...
        return f"<h1>Error loading template</h1><p>Ensure your index.html is inside a folder named 'templates' in your GitHub repo.</p><p>Error: {str(e)}</p>", 500
...
def ban_account():
    if game_version is None:
        return jsonify({'success': False, 'error': 'Vercel সার্ভারে game_version.py ফাইলটি নেই! দয়া করে এটি গিটহাবে আপলোড করুন।'}), 500
    ...
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'error': "গ্যারেনা সার্ভারের সাথে সংযোগ স্থাপন করা যাচ্ছে না।"}), 500
        ...
        try:
            ...
            if not response.ok:
                return jsonify({'success': False, 'error': "গেম সার্ভার লগইন রিকোয়েস্ট প্রত্যাখ্যান করেছে।"}), 500
        except Exception:
        ...
        # Completely removed the need for MajorLogin_res_pb2
        try:
        ...
        if not parsed_data:
            return jsonify({'success': False, 'error': "সার্ভার রেসপন্স ডিকোড করা সম্ভব হয়নি।"}), 500

        # Extract required fields directly from Custom Protobuf Parser
        account_jwt = parsed_data.get(8)
        resp_key = parsed_data.get(22)
        resp_iv = parsed_data.get(23)

        if not account_jwt or not resp_key or not resp_iv:
            return jsonify({'success': False, 'error': "লগইন রেসপন্সে টোকেন বা এনক্রিপশন কী পাওয়া যায়নি।"}), 500

        if isinstance(account_jwt, bytes):
            account_jwt = account_jwt.decode('utf-8', errors='ignore')
        ...
        if field_21_value:
            ...
            payload = extract_jwt_payload_dict(account_jwt)
        ...
        try:
            ...
            response = send_once(online_ip, online_port, payload_bytes, recv_timeout=8.0)
        ...
        traceback.print_exc()
        return jsonify({'success': False, 'error': f"সিস্টেমে একটি অপ্রত্যাশিত ত্রুটি ঘটেছে: {str(e)}"}), 500
